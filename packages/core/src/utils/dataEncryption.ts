...
function getEncryptionKeys(encryptionKey?: string) {
  const decryptionKey = encryptionKey || encryptionSecret;

  if (!decryptionKey) {
    throw new Error("decryptionKey not defined");
  }
...
export function decryptData(
  ...
  try {
    ...
    try {
      return JSON.parse(decrypted) as string | number | Record<string, unknown>;
    } catch (e) {
      return decrypted;
    }
  } catch (e) {
